...
(
    input logic clk, rst, en, acc_clear_and_go, acc_clear,
    input logic signed [WEIGHTS*DATA_WIDTH-1:0] weights_in,
    input logic [WEIGHTS-1:0] spikes,
    
    output logic [2*(DATA_WIDTH+1)-1:0] out,
    output logic valid
    );
...
logic signed [DATA_WIDTH-1:0] weights [WEIGHTS-1:0];
logic [PIPE-1:0] en_shift;
logic signed [DATA_WIDTH-1:0] weights_a [SUM-1:0]; 
logic [DATA_WIDTH-1:0] spike_a [SUM-1:0]; 
logic [DATA_WIDTH-1:0] spike_b [SUM-1:0]; 
logic [DATA_WIDTH-1:0] anded_weights_a [SUM-1:0];     
logic [DATA_WIDTH-1:0] anded_weights_b [SUM-1:0];     
logic signed [DATA_WIDTH:0] double_adder_out [SUM-1:0];
logic double_adder_en;
logic [2*(DATA_WIDTH+1)-1:0] acc_out;
logic signed [DATA_WIDTH-1:0] weights_b [SUM-1:0];  
